<template>
  <div class="glass rounded-2xl p-6 shadow-sm border border-gray-100/50 hover:shadow-md transition-shadow duration-300 relative overflow-hidden group">
    ...
    <button 
      @click="copyQuote" 
      class="absolute top-1 right-1 p-4 rounded-xl text-gray-400 hover:text-forest active:bg-forest/10 transition-colors z-20 touch-manipulation"
      aria-label="Copy quote"
    >
      <Icon :icon="copied ? 'lucide:check' : 'lucide:copy'" class="w-5 h-5" :class="copied ? 'text-forest' : ''" />
    </button>

    <p class="text-base md:text-lg font-medium text-charcoal leading-relaxed mb-4 relative z-10 pr-6" :title="quote.text">"{{ quote.text }}"</p>
